//! Picker contents and geometry.
//!
//! The complete list is built eagerly from static emoji strings; row positions are derived
//! from fixed heights rather than measured.
...
impl Grid {
    ...
    /// Build search results while keeping the tab bar's section slots inactive.
    pub fn search(query: &str, tone: u8) -> Grid {
        let hits: Vec<&'static str> = emoji::search(query).iter().map(|e| e.toned(tone)).collect();
        let mut b = Builder::default();
    ...
    /// Return the first row at or below `y`.
    ...
    ///
    /// Preserve the column where possible; clamp it for short rows.
    ...
    /// Return the smallest scroll change that makes row `r` visible.
