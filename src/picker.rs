//! Picker state: query, selection, scroll position, and settings mode.
//!
//! The backend sends events here and the renderer reads the state, so this logic is testable
...
impl Picker {
    ...
    /// The closure returns whether the grid must be rebuilt.
    ...
    /// Move by one viewport minus one row.
...
mod tests {
    ...
    fn moving_down_pulls_the_viewport_along() {
        ...
        assert!(
            p.scroll > 0.0,
            "viewport should have followed the selection"
        );
    ...
    fn the_recents_cap_stays_inside_the_range_the_store_accepts() {
        ...
        p.setting = SETTINGS
            ...
            .unwrap();
        assert_eq!(
            p.adjust_setting(-1, 0, lo),
            Some(Action::SetRecentLimit(lo))
        );
